...
import { fetchRateInfo as apiFetchRateInfo, convertCurrency, fetchMultipleRates, saveConversion, getFeeSettings } from "@/lib/converter-api";
...
const CURRENCY_NAMES: Record<string, string> = {
  ...
  "BSC-T": "بي إس سي توكن (BSC-T)",
  TRX: "ترون (TRX)",
  TTB: "تي تي بي توكن (TTB)",
  TSLA: "سهم تسلا (TESLA)",
...
const CURRENCY_CATEGORIES = [
  { label: "العملات التقليدية", currencies: ["USD", "EUR", "GBP", "JPY", "NZD"] },
  { label: "المعادن والسلع", currencies: ["XAU", "XAG", "OIL", "XCU"] },
  { label: "العملات الرقمية", currencies: ["USDT", "BTC", "ETH", "DOGE", "SOL", "SHIB", "PEPE", "NEAR", "BSC-T", "TRX", "TTB"] },
...
export default function ConverterPage() {
  ...
  const fetchRateInfo = useCallback(async () => {
    ...
    setRateLoading(true);
    try {
      const rateData = await apiFetchRateInfo(fromCurrency, toCurrency);
      if (rateData) {
        setRateInfo({
          buy_price: rateData.buy_price,
          sell_price: rateData.sell_price,
          mid_rate: rateData.mid_rate,
  ...
  useEffect(() => {
    const fetchDepositCurrency = async () => {
      try {
        ...
        setDepositCurrency(settings.deposit_currency);
      } catch {
        // Default USD
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchWatchlistRates = async () => {
      try {
        const pairs = watchlist.map((item) => ({
          base_currency: item.fromCurrency,
          quote_currency: item.toCurrency,
        }));
        const ratesData = await fetchMultipleRates(pairs);
        const newRates: Record<string, { mid: number; buy: number; sell: number; is_live: boolean }> = {};
        watchlist.forEach((item, idx) => {
          const rate = ratesData[idx];
          if (rate) {
            newRates[item.id] = {
              mid: rate.mid_rate,
              buy: rate.buy_price,
  ...
  const doConvert = useCallback(
    async (isExpert = false): Promise<ConvertResult | null> => {
      ...
      try {
        const res = await convertCurrency(fromCurrency, toCurrency, parseFloat(amount));
        if (res) return res;
      } catch (err) {
        console.error(err);
        if (!isExpert)
  ...
  const doConvertDirection = useCallback(
    async (
      ...
      try {
        return await convertCurrency(from, to, amt);
      } catch (err) {
        console.error(err);
      }
      return null;
  ...
  const autoSaveConversion = useCallback(async (res: ConvertResult) => {
    if (!user) return;
    await saveConversion(res, depositCurrency);
  ...
  const handleSaveConversion = async () => {
    if (!result) return;
    try {
      await saveConversion(result, depositCurrency);
        ...
        {result ? (
          <div className="bg-[#111827] rounded-xl p-5 space-y-4 border border-green-500/20 flex-1">
            ...
            <div className="text-center py-4">
              ...
              {result.fee_amount > 0 && (
                <p className="text-amber-400 text-sm mt-1">
                  - رسوم ثابتة: {formatFee(result.fee_amount, result.to_currency)}{" "}
                  {result.to_currency}
                </p>
              )}
            ...
            <div className="space-y-2 border-t border-[#374151] pt-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">الرسوم الثابتة</span>
                <span className="font-mono text-amber-400">
                  {result.fee_amount > 0
                    ? `${formatFee(result.fee_amount, result.to_currency)} ${result.to_currency} (مبلغ ثابت)`
                    : "بدون رسوم"}
                </span>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-gray-400">
              تحويل فوري بين العملات والأسهم بأسعار السوق العالمي مع رسوم ثابتة
            </p>
          </div>
                        ...
                        >
                          ...
                          {entry.executed && (
                            <div className="bg-[#0D1117] rounded-lg p-3 space-y-2 border border-[#1F2937]">
                              ...
                              <div className="flex items-center justify-between text-sm">
                                <span className="text-amber-400">الرسوم الثابتة</span>
                                <span className="font-mono text-amber-400">
                                  {entry.result.fee_amount > 0
                                    ? `- ${formatFee(
                                        entry.result.fee_amount,
                                        entry.result.to_currency
                                      )} ${entry.result.to_currency} (ثابت)`
                                    : "بدون رسوم"}
                                </span>
                              </div>
