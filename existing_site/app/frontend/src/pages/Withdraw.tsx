...
const CATEGORIES = [
  { id: "all", label: "الكل", icon: <Globe className="h-3.5 w-3.5" />, count: WITHDRAWAL_METHODS.length },
  { id: "wallets", label: "محافظ إلكترونية", icon: <Wallet className="h-3.5 w-3.5" />, count: WITHDRAWAL_METHODS.filter(m => m.category === "wallets").length },
  { id: "crypto", label: "عملات رقمية", icon: <Bitcoin className="h-3.5 w-3.5" />, count: WITHDRAWAL_METHODS.filter(m => m.category === "crypto").length },
  { id: "gateways", label: "بوابات دفع", icon: <CreditCard className="h-3.5 w-3.5" />, count: WITHDRAWAL_METHODS.filter(m => m.category === "gateways").length },
  { id: "transfers", label: "تحويلات بنكية", icon: <Send className="h-3.5 w-3.5" />, count: WITHDRAWAL_METHODS.filter(m => m.category === "transfers").length },
];
...
export default function WithdrawPage() {
  ...
  const fetchWithdrawals = async () => {
    try {
      const response = await client.entities.withdrawals.query({
        query: {},
        sort: "-created_at",
        limit: 20,
      });
  ...
  const fetchBalance = async () => {
    try {
      ...
      if (response?.data?.items) {
        const totalFees = response.data.items.reduce(
          (sum: number, conv: any) => sum + (conv.fee_amount || 0),
          0
        );
        const wdResponse = await client.entities.withdrawals.query({
          query: { status: "completed" },
          limit: 200,
        });
        const totalWithdrawn = (wdResponse?.data?.items || []).reduce(
          (sum: number, wd: any) => sum + (wd.amount || 0),
          0
        );
        setAvailableBalance(Math.max(totalFees - totalWithdrawn, 0));
      }
    } catch (err) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      await client.entities.withdrawals.create({
        data: {
      ...
      toast.success("تم إرسال طلب السحب بنجاح! سيتم معالجته قريباً.");
      setAmount("");
      setWalletAddress("");
      setBankName("");
      setIban("");
  ...
  const filteredMethods =
    activeCategory === "all"
      ? WITHDRAWAL_METHODS
      : WITHDRAWAL_METHODS.filter((m) => m.category === activeCategory);
    ...
    return (
      <div className="min-h-screen bg-[#0A0F1C] text-white" dir="rtl">
        ...
        <div className="max-w-4xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-gray-400 mb-6">يرجى تسجيل الدخول لسحب الأرباح</p>
          <Button
    ...
    <div className="min-h-screen bg-[#0A0F1C] text-white" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          <ArrowDownToLine className="h-7 w-7 text-green-400" />
          <h1 className="text-2xl font-bold">سحب الأرباح</h1>
        </div>
        <p className="text-gray-400 text-sm mb-8">
          اسحب أرباحك إلى أي بنك إلكتروني أو محفظة رقمية أو حساب بنكي —{" "}
          <span className="text-amber-400 font-semibold">
            {WITHDRAWAL_METHODS.length} طريقة سحب متاحة
          </span>
        </p>

        {/* Balance Card - Full Width */}
        <Card className="bg-gradient-to-r from-green-900/30 to-emerald-900/30 border-green-500/30 p-6 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-400 text-sm">الرصيد المتاح للسحب</p>
        ...
        <Card className="bg-[#1F2937] border-[#374151] p-6 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-lg">اختر طريقة السحب</h2>
            <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30 mr-auto">
              {WITHDRAWAL_METHODS.length} طريقة
            </Badge>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {filteredMethods.map((method) => (
              ...
              >
                ...
                {selectedMethod === method.id && (
                  <CheckCircle2 className="absolute top-1.5 left-1.5 h-4 w-4 text-green-400" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className="bg-[#1F2937] border-[#374151] p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="font-bold text-lg">طلب سحب جديد</h2>
                {selectedMethodData && (
                  <Badge
          ...
          {/* Withdrawal History */}
          <div>
            <h2 className="font-bold text-lg mb-4">سجل السحوبات</h2>
            {withdrawals.length === 0 ? (
              <Card className="bg-[#1F2937] border-[#374151] p-8 text-center">
                <ArrowDownToLine className="h-10 w-10 text-gray-600 mx-auto mb-3" />
                <p className="text-gray-400 text-sm">لا توجد سحوبات سابقة</p>
              </Card>
            ) : (
              <div className="space-y-3">
                {withdrawals.map((wd) => {
                  const status =
                    statusConfig[wd.status] || statusConfig.pending;
                  const methodData = WITHDRAWAL_METHODS.find(
                    (m) => m.id === wd.payment_method
                  );
                  return (
                    <Card
                      key={wd.id}
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {methodData?.name || wd.payment_method}
                            </p>
                          </div>
                        </div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {wd.created_at
                          ? new Date(wd.created_at).toLocaleDateString("ar-SA")
                          : "-"}
                      </p>
                    </Card>
                  );
