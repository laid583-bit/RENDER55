...
export default function AdminPage() {
  ...
  const handleUpdateFee = async (pairId: number) => {
    ...
    try {
      ...
      toast.error("حدث خطأ أثناء تحديث النسبة");
    }
  };
  ...
  const handleApplyGlobalFee = async () => {
    if (globalFeePercentage < 0) return;
    ...
    try {
      ...
      await fetchData();
      toast.success(`تم تطبيق نسبة ${globalFeePercentage}% على جميع الأزواج بنجاح!`);
    } catch (err) {
      console.error("Error applying global fee:", err);
      toast.error("حدث خطأ أثناء تطبيق النسبة الموحدة");
    }
          ...
          <TabsContent value="fees" className="space-y-3">
            ...
            <Card className="bg-[#1F2937] border-[#374151] p-4">
              <div className="space-y-2">
                <p className="text-amber-400 text-sm font-semibold">
                  ⚡ نظام الرسوم بالنسبة المئوية (%)
                </p>
                <p className="text-gray-400 text-sm">
                  يتم خصم نسبة مئوية من المبلغ المحول تلقائياً عند كل عملية تحويل.
                  مثال: إذا كانت النسبة 2% والمبلغ المحول 1000، يتم خصم 20 كرسوم.
                </p>
              </div>
            </Card>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-amber-400">
                    تعيين نسبة موحدة لجميع الأزواج
                  </h3>
                </div>
                <p className="text-gray-400 text-sm">
                  اختر نسبة جاهزة أو أدخل نسبة مخصصة لتطبيقها على جميع أزواج العملات دفعة واحدة
                </p>
                <div className="flex flex-wrap gap-2">
                  {[0.33, 0.5, 1, 1.5, 2, 3, 5].map((pct) => (
                    <Button
                      key={pct}
                      variant="outline"
                      size="sm"
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 max-w-[200px]">
                    <Input
                      ...
                      className="bg-[#111827] border-amber-500/30 text-white font-mono pl-8 text-lg"
                    />
            ...
            <div className="space-y-3">
              <p className="text-gray-400 text-sm font-semibold px-1">
                📋 إعدادات الرسوم لكل زوج (يمكنك تخصيص نسبة مختلفة لكل زوج)
              </p>
              {pairs.map((pair) => {
                const fee = fees.find((f) => f.pair_id === pair.id);
                const currentPct = fee?.fee_percentage || 0;
                const PRESET_PERCENTAGES = [0.5, 1, 2, 3, 5];
                return (
                  <Card
                    key={pair.id}
                    className="bg-[#1F2937] border-[#374151]"
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-400 mt-0.5">
                                النسبة الحالية:{" "}
                                {currentPct > 0 ? (
                                  <span className="text-amber-400 font-mono font-bold text-sm">{currentPct}%</span>
                                ) : (
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="relative">
                            <Input
                              ...
                              placeholder="مخصص"
                              className="w-24 h-7 bg-[#111827] border-amber-500/30 text-white font-mono text-xs pl-6"
                            />
                          ...
                          <Button
                            ...
                            size="sm"
                            className="bg-amber-500 hover:bg-amber-600 text-black font-bold h-7 text-xs"
                          >
